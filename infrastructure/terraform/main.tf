...
resource "aws_security_group" "final_task_sg" {
  ...
  ingress {
    description = "Alertmanager (Task 7)"
    from_port   = 9093
    to_port     = 9093
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
  ...
  ingress {
    description = "Node Exporter (Task 7)"
    from_port   = 9100
    to_port     = 9100
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
